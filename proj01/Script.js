...
let formaPagamento = prompt("Escolha a forma de pagamento (À vista no PIX, Cartão ou Financiamento):");
let valorFinal;
let valorParcela;

if (formaPagamento === "Á vista no PIX") {
    ...
    if (parcelas > 0 && parcelas <= 24) {
        ...
        alert("Quantidades de parcelas inválida para Cartão.");
    }
    ...
    if (parcelas > 0 && parcelas <= 48) {
        ...
        valorParcela = valorFinal / parcelas;
        alert(`Pagamento dia financiamento. Total: R$ ${valorFinal.toFixed(2)} em ${parcelas}x de R$ ${valorParcela.toFixed(2)});`)
    } else {
